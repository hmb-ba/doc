...
The log subsystem is responsible for handling persistence of the messages being
...
After this follows the explanation of the code for the  most important
functionalities where information about design decisions and potential threats
are given.
...
is this case will be the API layer. The \textit{LogManager} is then able to use
exposed functionalities of the hidden (displayed as dashed) \textit{Log} or
\textit{Index} modules.
...
As for now, the functionalities the \textit{LogManager} provides it to create a
...
\textit{Index} module itself. The last and perhaps most special thing that is
exposed is the \lstinline{State} which is the representation of the
current Log and Index, as discussed later in this section.
...
index entry is created only after \textit{4096 Bytes} of log data.
...
were defined. Those are only used in the log subsystem and are defined within
the \textit{LogConfig}. The following table gives an overview of these types:
...
speak -- is not only very difficult to manage but also extremely slow (see
\todo{separate section of stateless approach}), a more convenient and powerful
approach needs to be introduced to handle data on a running broker system.
Therefore an efficient implementation of a key-value data structure, namely
...
The \lstinline{LogState} represents an MVar holding a list of maps. Each of which whos key is a tuple of
\lstinline{String} and \lstinline{Int} representing the topic and partition
number combination. The value for those maps is the actual log, which is a list
...
MVar containing a list maps and also the key remains the same. However the value
of those maps will hold a list of \lstinline{OffsetPosition}, whose name is
adapted form Apache Kafka and is a tuple of the \lstinline{RelativeIndex} and
\lstinline{PhysicalPosition} of the related log entry.
...
-- of the related log or index file.  The thing that is still missing now, is either in
...
\begin{figure}[H]
    \centering
     \begin{sequencediagram}
        ...
        \end{call}
        \begin{call}
            {manager}{(2) Determine last offset}{manager}{}
        \end{call}
        \begin{call}
...
append ((Log.LogState ls, Index.IndexState is), t, p, ms) = do
  ...
  let llo = fromMaybe 0 (Log.lastOffset log) -- (2)
  let newLog = log ++ Log.continueOffset (llo + 1) msA -- (3)
