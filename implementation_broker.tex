...
to the client. For sending data over the socket, function sendAll from \fnurl{Network.Socket.ByteString.Lazy}
...
unexpected errors occuring. In this case, the underlying C socket implementation will return a result of \textit{-1} which will then result in an\fnurl{IOError}{https://hackage.haskell.org/package/base-4.4.1.0/docs/System-IO-Error.html}. 
...
are defined. These are only used in the log subsystem and are defined within
...
An incoming request--which at this point already parsed--contains the
...
we also provided some optimizations in terms of I/O. \todo{Kafka: a
