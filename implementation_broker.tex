...
steps come in between they will contribute significantly to the concept of
Apache Kafka's Log system \todo{ref to kafka log}. This section will describe
...
can identify the location of the related log file. Given the location we do
not know which file we actually have to write the messages to at this point.
The file we want to write to is the one named after the highest offset,
which is why we do not get around reading the content of this directory and
proceed string transformations to identify the correct offset and thus the
file to write to. Handling the case of where no file exists follows
to the creation of a new log with the offset of zero.

Here Haskell can perform it's beauty. To get the list of all offsets within
...
The offset can be mapped over a filtered list of strings, which will
give the list of all offsets within the directory. The filter function
basically omits the files other than \textit{".log"} as well as the root
...
Finally, the highest offset or zero will be given:
