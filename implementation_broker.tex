...
During the process of listening to an open connection and reading incomming byte-stream there is a chance 
...
Errors at this stage will be handled using the \textit{Socket Error Handler} and do not result in any reponse to the client who has initiated the request that result in an error as the socket connection may be broken. Instead, the error is simply catched and can then be reported using a separate logging framework such as \fnurl{hslogger}{https://hackage.haskell.org/package/hslogger}. At worst, the socket connection has to be closed.
...
by parsing the request, by writing data to the disk exectued by the log subsystem, or even by producing 
the reponse message at last. All of the mentioned cases and those who fell into the related category, will be handled by the \textit{RequestErrorHandler} where a set of possible errors or category of error is defined
and will result in an appropriate reponse message to the client.
