...
serious as fault tolerance and reliablity are key requirements \todo{ref:
...
edge cases they have to be consiered will be uncovered in order to provide
...
errors may occur. This will also involve part of the concetp behind the error
...
\subsection{haskell error handling....}
...
\subsubsection{Defining Error Types}

Error types related to the networking layer are simply distinguished between an error
occurred either in the receiving or responding process.
...
error will not be handled directly, but instead be caught by the request handle
who is now in charge to map this error to one of the defined
...
\subsubsection{Handlers in detail}
