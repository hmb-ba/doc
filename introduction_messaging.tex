...
\todo{ref} which still are relevant today. Messaging mainly faces the
following:
...
\begin{enumerate}
   ...
   %\item{\textbf{The Latency is zero:o:} \hfill \\
   %     .
   %     }
    ...
    %\item{\textbf{The network is secure:} \hfill \\
    %       .
    %    }
    ...
    %\item{\textbf{There is one administrator}  \hfill \\
    %    .
    %    }
    %\item{\textbf{Transport cost is zero } \hfill \\
    %    .
    %    }
...
% Facing the problem of a slow network, Collaborating applications must agree on a common message forma
