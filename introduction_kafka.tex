...
Apache Kafka stores all data immediately to a persistent log\todo{ref to log} 
and therefore relies heavily on the file system \todo{gls}.
Instead of flushing incoming messages from producers directly to the disks, the
data is stored as a compact byte structure within the page cache \todo{gls}.
\\ \\
...
optimization but also provides read-ahead und write-behind\todo{gls} techniques.
The latter will prefetch data in large block multiples and group smaller logical
writes into large physical writes. Thus, a SATA RAID-5 \todo{gls} of six 7200rpm
