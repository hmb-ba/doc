...
to the functionalities Apache Kafka provides, we describe in the following the
...
Apache Kafka handles its own log for every topic whereas the log contains all
...
version including all the updates done in past. Describing every replica by the
...
consumption of messages. Every consumer knows its actual offset and increases it
...
followers. Instead of using the common algorithm of quroum replication \todo{gls}, it
...
crashes all surviving followers register itself in Zookeeper. The replica that
...
of filesystem writes but rather relies fully on the file system (as described in
