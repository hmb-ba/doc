...
Firts initialize stream socket an open connection to a given host address and port (broker
is running). If connection succeeded, retrieve clientId via console, request header can be build:
\begin{lstlisting}[caption={Initialize client}]
...
Retrieve requested topic name and partition from console. After, each given
console input is send to the broker as single request. Therefore the types
and function \lstinline{sendRequest} from client library is used. After sending
...
\begin{lstlisting}[caption={Producer client example}]
 ...
 let p = read partition :: Int
  ...
  forever $ do 
    ...
    let prReq = C.Produce 
                    requestHeader
                    [ C.ToTopic t [ C.ToPart p [(C.stringToData input)]]]
    C.sendRequest sock $ prReq

    input <- SBL.recv sock 4096
    let response = C.decodePrResponse input
    print response 
\end{lstlisting}
